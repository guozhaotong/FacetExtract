...
//		System.out.println(facetsOfOneNode("Binary_tree"));
		allAncientFacet("Data_structure");
		System.out.println("done.");
	}
	
	@SuppressWarnings("deprecation")
	public static void allAncientFacet(String root) {
		File dirfile =new File(oriPath + "complementation\\ancient");    
		if  (!dirfile .exists()  && !dirfile .isDirectory())      
		{       
		    dirfile .mkdir();    
		}
		dirfile =new File(oriPath + "complementation\\ancient\\inherit\\");    
		if  (!dirfile .exists()  && !dirfile .isDirectory())      
		{       
		    dirfile .mkdir();    
		}
		dirfile =new File(oriPath + "complementation\\ancient\\statistics\\");    
		if  (!dirfile .exists()  && !dirfile .isDirectory())      
		{       
		    dirfile .mkdir();    
		}
		...
		for(String name : fileName)
		{
			...
			parent = FindRelationship.getRelation(name,root).getParentNodes();
			...
			try {
				FileUtils.write(new File(oriPath + "complementation\\ancient\\statistics\\" + name + ".txt"), cont);
				FileUtils.write(new File(oriPath + "complementation\\ancient\\inherit\\" + name + ".txt"), allf);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
